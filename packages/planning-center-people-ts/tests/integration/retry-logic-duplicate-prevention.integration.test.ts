...
describe('Retry Logic - Duplicate Prevention Integration Test', () => {
    ...
    describe('Duplicate Prevention with Retry Logic', () => {
        it('should find existing person after contact verification delay with retry logic', async () => {
            ...
            // getEmails and getPhoneNumbers return flattened resources - attributes are at top level
            const hasEmail = emails.data.some(e => e.address === testEmail);
            const hasPhone = phones.data.some(p => p.number === testPhone);
            ...
            try {
                ...
                throw new Error(
                    ...
                    `or there's an issue with the retry logic. Error: ${(error as Error).message}`
                );
            }
        ...
        it('should demonstrate the bug scenario without retry logic (may pass if PCO is fast)', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(Error);
                expect((error as Error).message).toContain('No matching person found');
            }
        ...
        it('should fail without retry logic when contacts are not verified (demonstrates the bug)', async () => {
            ...
            try {
                ...
                expect(errorMessage).toContain('No matching person found');
            }
        }, 120000);
