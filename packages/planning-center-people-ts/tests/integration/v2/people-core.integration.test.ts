...
describe('v2.0.0 People Core API Integration Tests', () => {
    ...
    describe('v2.0 Read Operations', () => {
        it('should get people list with proper typing using v2.0 API', async () => {
            ...
            // Validate PersonAttributes - getPage returns flattened resources, attributes are at top level
            ...
            if ('emails' in person) {
                const emails = Array.isArray(person.emails) ? person.emails : person.emails ? [person.emails] : [];
                expect(emails.length).toBeGreaterThanOrEqual(0);
            }
            if ('phone_numbers' in person) {
                const phones = Array.isArray(person.phone_numbers) ? person.phone_numbers : person.phone_numbers ? [person.phone_numbers] : [];
                expect(phones.length).toBeGreaterThanOrEqual(0);
            }
            if ('primary_campus' in person) {
                expect(person.primary_campus).toBeDefined();
            }
            if ('gender' in person) {
            ...
            // Note: getPage may not always include resources even if requested
    ...
    describe('v2.0 Built-in Pagination', () => {
        it('should get all pages using getAll() method', async () => {
            ...
            // Validate that we got people (getPage with perPage: 10 returns up to 10 per page)
            const totalCount = typeof result.meta?.total_count === 'number' ? result.meta.total_count : 0;
            if (totalCount > 10) {
                // If there are more than 10 total, getPage with perPage: 10 should return exactly 10 (or less if total is less)
                expect(result.data.length).toBeLessThanOrEqual(10);
                expect(result.data.length).toBeGreaterThan(0);
            } else {
                // If total is 10 or less, we should get all of them
