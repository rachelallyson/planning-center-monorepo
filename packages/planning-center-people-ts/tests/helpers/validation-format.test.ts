...
describe('helpers: validation and formatting', () => {
  it('validatePersonData flags invalid email, phone, and birthdate', () => {
    const result = validatePersonData({ email: 'bad@@', phone: 'abc', birthdate: 'not-a-date' });
  ...
  it('validatePersonData passes valid inputs', () => {
    const result = validatePersonData({ email: 'good@gmail.com', phone: '+15551234567', birthdate: '2000-01-01' });
  ...
  it('formatPersonName builds from nickname, first, last', () => {
    expect(formatPersonName({ nickname: 'Ace', last_name: 'Smith' })).toBe('Ace Smith');
    expect(formatPersonName({ first_name: 'Ada' })).toBe('Ada');
    expect(formatPersonName({ last_name: 'Lovelace' })).toBe('Lovelace');
