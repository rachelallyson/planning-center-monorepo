...
## Step 4: Pagination and debug

Use `getPage()` for a single page or `getAll()` for all pages. Set `config.debug: true` for request logging.

### Debug logging and rate limit info

Enable request logging with `config.debug` and check rate limit state with `getRateLimitInfo()`:

```typescript
// Optional: enable debug logging when creating the client
const client = new PcoClient({
  auth: { /* ... */ },
  debug: true
});
...
**See [Debug & Observability](./event-system.mdx) for more options (custom log sink, prefix, payloads).**
