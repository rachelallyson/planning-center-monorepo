...
const response = await client.people.getAll({ per_page: 50 });

// Get maximum items per page
const response = await client.people.getAll({ per_page: 100 });
```
...
```typescript
const response = await client.people.getAll({ per_page: 25 });
...
async function getAllPeople(client: PcoClient) {
  ...
  while (hasMore) {
    const response = await client.people.getAll({ per_page: 25, page });
    allPeople.push(...response.data);
    hasMore = response.links?.next !== undefined && response.links?.next !== null;
    page++;
  }
