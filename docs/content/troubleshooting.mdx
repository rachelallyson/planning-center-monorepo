...
try {
  await client.people.getAll({ per_page: 1 });
} catch (error) {
...
```typescript
// Enable debug logging to see request/response and errors
const client = new PcoClient({
  auth: { /* ... */ },
  debug: true
});

// Or wrap calls in try/catch and log
try {
  await client.people.getById('1');
} catch (error) {
...
3. See [Error Handling](/guides/error-handling) and [Configuration](/reference/config) for People/Check-Ins specifics
